	...
	implements com.traclabs.biosim.idl.simulation.crew.CrewGroup
{
	...
	public float getDryWasteProduced()
	{
		...
		if(! this._is_local())
		{
			...
			try
			{
				org.omg.CORBA.portable.OutputStream _os = _request( "getDryWasteProduced", true);
				_is = _invoke(_os);
				float _result = _is.read_float();
				return _result;
			}
		...
		{
			org.omg.CORBA.portable.ServantObject _so = _servant_preinvoke( "getDryWasteProduced", _opsClass );
			if( _so == null )
				throw new org.omg.CORBA.UNKNOWN("local invocations not supported!");
			CrewGroupOperations _localServant = (CrewGroupOperations)_so.servant;
			float _result;			try
			{
			_result = _localServant.getDryWasteProduced();
			}
			finally
			{
				_servant_postinvoke(_so);
