	...
	implements com.traclabs.biosim.idl.simulation.framework.InfluentValve
{
	...
	public void clearMalfunction(long id)
	{
		...
		else
		{
			...
			InfluentValveOperations _localServant = (InfluentValveOperations)_so.servant;
			try
			{
	...
	public int getIndexOfInfluentStore()
	{
		...
		if(! this._is_local())
		{
			...
			try
			{
				org.omg.CORBA.portable.OutputStream _os = _request( "getIndexOfInfluentStore", true);
				_is = _invoke(_os);
				int _result = _is.read_long();
				return _result;
			}
		...
		{
			org.omg.CORBA.portable.ServantObject _so = _servant_preinvoke( "getIndexOfInfluentStore", _opsClass );
			if( _so == null )
				throw new org.omg.CORBA.UNKNOWN("local invocations not supported!");
			InfluentValveOperations _localServant = (InfluentValveOperations)_so.servant;
			int _result;			try
			{
			_result = _localServant.getIndexOfInfluentStore();
			}
			finally
			{
				_servant_postinvoke(_so);
	...
	public void setIndexOfInfluentStore(int pIndexOfEffluentStore)
	{
		while(true)
		{
		if(! this._is_local())
		{
			...
			try
			{
				org.omg.CORBA.portable.OutputStream _os = _request( "setIndexOfInfluentStore", true);
		...
		{
			org.omg.CORBA.portable.ServantObject _so = _servant_preinvoke( "setIndexOfInfluentStore", _opsClass );
			if( _so == null )
				throw new org.omg.CORBA.UNKNOWN("local invocations not supported!");
			InfluentValveOperations _localServant = (InfluentValveOperations)_so.servant;
			try
			{
			_localServant.setIndexOfInfluentStore(pIndexOfEffluentStore);
			}
			finally
			{
				_servant_postinvoke(_so);
